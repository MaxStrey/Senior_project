...
class BallTracker:
    ...
    def detect_ball(self, frame):
        """Detect the brightest circular moving object in the frame.

        Pipeline (all operations on 1280×800 uint8 grayscale):
          1. Convert BGR→Gray if needed          (~1 ms)
          2. CLAHE histogram equalization         (~2 ms)
          3. Frame differencing (motion mask)     (~0.5 ms)
          4. GaussianBlur on diff                 (~2 ms)
          5. Threshold diff → binary motion mask  (~0.2 ms)
          6. Threshold equalized → bright mask    (~0.2 ms)
          7. AND(motion, bright)                  (~0.3 ms)
          8. Morphological open + close           (~2 ms)
          9. findContours + scoring               (~1-3 ms)
                                         Total ≈ 10-15 ms
        ...
        try:
            ...
            # Step 1: Convert to grayscale if the frame is BGR.
            # Your OV9281 is monochrome, but OpenCV's MJPG decode path
            # expands it to 3-channel BGR anyway.  This converts back.
            if len(frame.shape) == 3:
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            else:
                gray = frame

            # Step 2: CLAHE (Contrast-Limited Adaptive Histogram
            # Equalization).  This normalizes local contrast so the
            # brightness threshold adapts to uneven lighting.
            gray_eq = self.clahe.apply(gray)

            # Step 3: Frame differencing.  If there's no previous frame
            # yet (first frame), just store and return.
            ...
            # Step 4-5: Blur the diff to suppress noise, then threshold
            ...
            # Step 6: Adaptive brightness threshold.
            # T = mean(gray_eq) + k * std(gray_eq)
            # Pixels brighter than T are candidates for the white ball.
            ...
            # Step 7: AND the motion and brightness masks.
            # Only regions that are BOTH moving AND bright survive.
            mask = cv2.bitwise_and(motion, bright)

            # Step 8: Morphological cleanup.
            # OPEN removes small noise blobs (erosion then dilation).
            # CLOSE fills small holes inside the ball blob.
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
            mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=1)
            mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel, iterations=2)

            # Step 9: Find contours and score them.
            ...
            for contour in contours:
                ...
                # Circularity = 4π·A / P².  A perfect circle has C=1.
                ...
                # Score = circularity × area.  Prefers large, round blobs.
...
# ──────────────────────────────────────────────────────────────────────
def processing_thread_fn(tracker, calibration_state):
...
# ──────────────────────────────────────────────────────────────────────
def draw_tracking_overlay(frame, tracker, calibration_state, show_hud=True):
    try:
        if frame is None:
            return None
        ...
        if show_hud:
            ...
            cv2.putText(
                disp,
                f"{calibration_state['table_width_m'] * 100.0:.0f} x "
                f"{calibration_state['table_height_m'] * 100.0:.0f} cm",
                (panel_x + 190, panel_y + 72),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.45,
                (180, 220, 180),
                1,
...
def ball_tracker_live(
    ...
    try:
        ...
        proc_thread = threading.Thread(
            ...
            args=(tracker, calibration_state),
            daemon=True,
        )
        proc_thread.start()
        ...
        while running:
            ...
            disp = draw_tracking_overlay(
                frame, tracker, calibration_state, show_hud=show_hud
            )
